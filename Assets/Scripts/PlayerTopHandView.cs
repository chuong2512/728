...
public class PlayerTopHandView : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider collider)
	{
		if (collider.tag.Equals("ball"))
		{
			...
			trap.transform.localEulerAngles = new Vector3(-127.418f, 90f, -90f);
			...
			{
				float num = 5f - this.m_PlayerView.transform.localPosition.y;
				if (num <= 0f)
				{
					num = 0f;
				}
				...
				float num4 = UnityEngine.Random.value * 5f + 3f;
				float num5 = UnityEngine.Random.value * 10f + 20f;
				component.AddForce(new Vector3(-num3 * num2, (this.m_PlayerView.m_body.velocity.y + num4) * num2, 0f), ForceMode.VelocityChange);
				component.AddTorque(new Vector3(-num3 * 2f, (this.m_PlayerView.m_body.velocity.y + num4) * 2f, num5 * 2f), ForceMode.VelocityChange);
